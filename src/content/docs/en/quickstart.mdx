...
# Database (Required)
DATABASE_URL="postgresql://username:password@localhost:5432/ai-saas-template"
```
...
# Inside psql, run:
CREATE ROLE ai_user WITH LOGIN PASSWORD 'YourPassword';
CREATE DATABASE ai_saas_template OWNER ai_user;
...
Visit `http://localhost:3000/en/sign-up` to test:
...
Test the payment flow at `http://localhost:3000/en/pricing`:
...
Try file operations at `http://localhost:3000/en/dashboard/files`:
